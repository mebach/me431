...
class dataPlotter:
    ...
    def update(self, t, reference, states, ctrl):
        ...
        self.z_ref_history.append(reference)  # reference base position
        self.z_history.append(states.item(0))  # rod angle (converted to degrees)
        self.F_history.append(ctrl)  # force on the base

        # update the plots with associated histories
        self.handle[0].update(self.time_history, [self.z_history, self.z_ref_history])
        self.handle[1].update(self.time_history, [self.F_history])
