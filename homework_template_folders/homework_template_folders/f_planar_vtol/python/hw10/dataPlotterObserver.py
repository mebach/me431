...
class dataPlotterObserver:
    def __init__(self):
        ...
        self.handle = []
        self.handle.append(myPlot(self.ax[0], ylabel='z (m)', title='VTOL Data'))
        self.handle.append(myPlot(self.ax[1], ylabel='h (m)'))
        self.handle.append(myPlot(self.ax[2], ylabel='theta (deg)'))
        self.handle.append(myPlot(self.ax[3], ylabel='z_dot (m/s)'))
        self.handle.append(myPlot(self.ax[4], ylabel='h_dot (m/s'))
        self.handle.append(myPlot(self.ax[5], ylabel='theta_dot (deg/s)'))
        # self.handle.append(myPlot(self.ax[6], xlabel='t(s)',
        #                           ylabel='d'))
    ...
    def update(self, t, x, x_hat, d, d_hat):
        ...
        self.h_hat_history.append(x_hat.item(1))
        self.theta_hat_history.append(x_hat.item(2))
        self.z_hat_dot_history.append(x_hat.item(3))
        self.h_hat_dot_history.append(x_hat.item(4))
        self.theta_hat_dot_history.append(x_hat.item(5))
        ...
        self.handle[3].update(self.time_history, [self.z_dot_history, self.z_hat_dot_history])
        self.handle[4].update(self.time_history, [self.h_dot_history, self.h_hat_dot_history])
        self.handle[5].update(self.time_history, [self.theta_dot_history, self.theta_hat_dot_history])
        # self.handle[6].update(self.time_history, [self.d_history, self.d_hat_history])
